...
{
    public static readonly string MemberKind = $"global::Antelcat.AutoGen.ComponentModel.Diagnostic.AutoReport.{nameof(MemberKind)}";
                                     ...
                                     .Where(x => x.DeclaredAccessibility.IsIncludedIn(access)))
                        {
                            switch (member)
                            {
                                case IFieldSymbol { IsImplicitlyDeclared: false } field:
                                    sb.Add(Method(field.MetadataName, field.Type, AutoReport.MemberKind.Property,
                                        $"() => {arg}.{field.Name}"));
                                    break;
                                case IPropertySymbol property:
                                {
                                    ...
                                    {
                                        sb.Add(Method(property.MetadataName,property.Type,AutoReport.MemberKind.Property,$"() => {arg}.{property.Name}"));
                                    }
                                ...
                                {
                                    if (methodSymbol is { MethodKind: MethodKind.Ordinary, Parameters.Length: 0 })
                                    {
                                        sb.Add(Method(methodSymbol.MetadataName, methodSymbol.ReturnType,
                                            AutoReport.MemberKind.Method,
                                            methodSymbol.ReturnsVoid ? "null" : $"() => {arg}.{methodSymbol.Name}()"));
                                    }
                                ...
                                case INamedTypeSymbol typeSymbol:
                                {
                                    ...
                                    {
                                        sb.Add(Method(typeSymbol.MetadataName,typeSymbol,AutoReport.MemberKind.NestedType,$"() => typeof({typeSymbol.GetFullyQualifiedName()})"));
                                    }

                                    break;
                                }
                            }
                            StatementSyntax Method(string name,ITypeSymbol type, AutoReport.MemberKind kind,string getter)
                            {
                                return ParseStatement(
                                    $"{report}(\"{name}\", typeof({type.GetFullyQualifiedName()}), {MemberKind}.{kind.ToString()}, {getter});");
                            }
                        }
